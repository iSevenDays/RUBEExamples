...
Scene* PinballRUBELayer::scene()
{
    ...
    PinballRUBELayer* layer = PinballRUBELayer::create();
...
void PinballRUBELayer::afterLoadProcessing(b2dJson* json)
{
    ...
    for (int i = 0; i < numFixtureNames; i++)
    {
        ...
        for (int k = 0; k < switchFixtures.size(); k++)
            switchFixtures[k]->SetUserData((intptr_t*)(i+1));
    }
    ...
    m_leftFlipperTouch = nullptr;
    m_rightFlipperTouch = nullptr;
...
{
    m_leftFlipperButtonFixture = nullptr;
    m_rightFlipperButtonFixture = nullptr;
    
    m_leftFlipperJoints.clear();
    m_rightFlipperJoints.clear();
    
    CC_SAFE_DELETE(m_leftFlipperTouch);
    CC_SAFE_DELETE(m_rightFlipperTouch);
...
void PinballRUBELayer::draw(cocos2d::Renderer *renderer, const kmMat4 &transform, bool transformUpdated)
{
    //don't do debug draw
}
...
void PinballRUBELayer::onTouchesEnded(const std::vector<Touch *> &touches, cocos2d::Event *event)
{
...
void PinballContactListener::BeginContact(b2Contact* contact)
{
    ...
    intptr_t tagA = (intptr_t) contact->GetFixtureA()->GetUserData();
    intptr_t tagB = (intptr_t) contact->GetFixtureB()->GetUserData();
