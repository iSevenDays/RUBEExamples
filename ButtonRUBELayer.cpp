...
void ButtonRUBELayer::draw(Renderer *renderer, const kmMat4& transform, bool transformUpdated)
{
    //no debug draw
}
...
void ButtonRUBELayer::doButtonTouch(Touch* touch)
{
    ...
    if ( bi && bi == m_touchedButton && allowButtonPresses() ) {
        ...
        /*else if ( bi->sceneClassType ) {
         ...
         [[Director sharedDirector] replaceScene:[bi->sceneClassType scene]];
         }*/
    }
}
void ButtonRUBELayer::onTouchesBegan(const std::vector<Touch *> &touches, cocos2d::Event *event)
{
...
void ButtonRUBELayer::onTouchesMoved(const std::vector<Touch *> &touches, cocos2d::Event *event)
{
    for(auto it = touches.begin(); it != touches.end(); it++)
    {
        Touch *touch = (Touch*)*it;;
...
void ButtonRUBELayer::onTouchesEnded(const std::vector<Touch *> &touches, cocos2d::Event *event)
{
